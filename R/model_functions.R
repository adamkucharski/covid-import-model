...
get_MLE <- function(){
  ...
  modelB.P <- gam(lik ~ s(rr,k=8) + s(decline,k=3) + s(imp,k=3) , data = rr_store,family = "gaussian") 
  x1_list <- seq(min(parameter_list$rr),max(parameter_list$rr),0.005)
  x2_list <- seq(min(parameter_list$decline),max(parameter_list$decline),0.0005)
  x3_list <- seq(min(parameter_list$imp),max(parameter_list$imp),0.01)
  ...
  rr_store %>% filter(imp==1)
  
  # Estimate MLE
  mle_val <- param_long[which(preds$fit==max(preds$fit)),]
  
  range_95 <- param_long[which(preds$fit>=(max(preds$fit)-1.92)),]
  ...
  # Run fitted model
  add_d <- 10
  output0 <- fit_R(mle_val$rr,run_n,add_days = add_d, import_f = mle_val$imp, daily_decline=mle_val$decline)
  
  list(out = output0, mle = mle_val, r95_rr = range_95_rr, r95_dec = range_95_decline, r95_imp = range_95_imp)
